...
class UserController extends BaseController {
			  ...
			  if ($v_code && $v_code == session('v_code')) {
			  	if ( $password != '' && $password == $confirm_password) {
			  		...
			  		if ( ! D("User")->isExist($username) ) {
						...
						if ($ret) {
					      $this->message("注册成功！",U('Showdoc/User/login'));
						}else{
		...
		if (!IS_POST) {
			...
			if ($cookie_token) {
				...
				if ($ret && $ret['cookie_token_expire'] > time() ) {
					...
					$this->message("自动登录成功！正在跳转...",U('Showdoc/Item/index'));
					exit();
				}
			}
		  $this->display ();
		  ...
		  if ($v_code && $v_code == session('v_code')) {
		    ...
		    if ($ret) {
	          ...
	          $this->message("登录成功！",U('Showdoc/Item/index'));
		    }else{
		      $this->message("用户名或密码不正确");
		    }
		...
		if (!IS_POST) {
		  ...
		  $this->display ();
			...
			if ($ret) {
					...
					if ($ret) {
						$this->message("修改成功！",U("Showdoc/Item/index"));
					}else{
						$this->message("修改失败！");
	...
	public function exist(){
		...
		$this->message("退出成功！",U('Showdoc/index/index'));
	}
}