...
class Alibankpay extends \Think\Pay\Pay {

    protected $gateway = 'https://mapi.alipay.com/gateway.do';
    protected $verify_url = 'http://notify.alipay.com/trade/notify_query.do';
    protected $configs = array(
        'email' => '2868445961@qq.com',
        'key' => 'd2jxioybf5r6j9yfzo4jof4ollg4rfit',
        'partner' => '2088021404956391'
    );

    public function check() {
        if (!$this->configs['email'] || !$this->configs['key'] || !$this->configs['partner']) {
            E("支付宝设置有误！");
        }
        return true;
    }
    ...
    public function buildRequestForm(\Think\Pay\PayVo $vo) {
        $param = array(
            ...
            'seller_email' => $this->configs['email'],
            'partner' => $this->configs['partner'],
           'notify_url' => 'http://www.gouchehui.com'. U("Home/Pay/notify", array('apitype' => 'alibankpay', 'method' => 'notify')),
            'return_url' => 'http://www.gouchehui.com'.U("Home/Pay/notify", array('apitype' => 'alibankpay', 'method' => 'notify')),
            'out_trade_no' => $vo['out_trade_no'],
           'subject' => $vo['title'],
            'body' => '商品支付',
            'total_fee' => $vo['money'],
            'defaultbank'=>$vo['defaultbank']            
        ...
        $param['sign'] = md5(substr($arg, 0, -1) . $this->configs['key']);
        $param['sign_type'] = 'MD5';
