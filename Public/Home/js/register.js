...
var GCH = GCH || {};
GCH.Register = GCH.Register || {};
GCH.Register.Aside = function(B) {
	B = B || {};
	var A = this;
	this.elementIds = B.elementIds || [""];
	this.elementNames = B.elementNames || [""];
	this.focusClass = B.focusClass || "G_input_focus";
...
var RegisterAside = new GCH.Register.Aside({
	elementIds: ["telephone", "txtPwd", "txtMobileCode"]
});
			...
			var D = this,
				C = $.extend({
					...
					data: {
						"popType": 0,
						"r": Math.random(),
						"telephone": D.option.mobileGeter(),
...
(function(A, B) {
	var C = function(D) {
			...
			this.option = B.extend({}, {
				...
				tabName: "mobile",
				defaultCurrentTabIdent: "li[data-tab='mobile']",
				onTabChange: null
			}, D);
			...
			this.lastTab = this.container.find(this.option.defaultCurrentTabIdent).next();
...
(function(A, B) {
	var C = function(F) {
			...
			var G, I = this,
				...
				F = this.option = B.extend(true, {
					...
					eventName: "blur",
					ValidatorOption: {
						url: "/index.php/Home/Public/userNameCheck",
						button: B("#submit"),
						form: B("#form1"),
						parameters: function() {
							return {
						...
						onItemFail: function(K) {
							...
							K.elements.addClass("G_input_error");
							if (B.browser.msie) {
								if (K.elements.data("brother")) {
									K.elements.data("brother").removeClass("input_text");
									K.elements.data("brother").addClass("G_input_error")
								}
							}
							if (J) {
								H.hide();
						...
						onItemSuccess: function(K) {
							K.elements.removeClass("G_input_error");
	...
	C.prototype = B.extend(C.prototype, {
		...
		ModelChangeDic: {
			"mobile": function() {
				this.validator.removeItem("telephone");
		...
		createAsideHtml: function(D) {
			return '<em class="G_wrong"><i class="icon_warn"></i>' + D + "</em>"
		},
		createAsidePassedHtml: function(D) {
			return '<em class="correct"><i class="icon_warn"></i>' + D + "</em>"
		}
	});
	A.Register = C
})(GCH, $);
/* 获取验证码 */
$(function() {
	var G = new GCH.ImgCoder(),
		E = new GCH.Register(),
		B = new GCH.AutoEmail({
			EmailList: GCH.AutoEmailList
		}),
		C = $("#a_CreateMobileCode"),
		A = $("#tishi_jym"),
		F = new GCH.MobileMsgCreator({
			isFirstRequest: GCH.IsFirstRequestMsg,
			isValidateMobile: true,
			...
			eventsCallbacks: {
				...
				fail: function(M) {
					var J = E.validator.getItem("txtMobileCode"),
						...
						L = {
							...
							"3": function() {
								K = E.validator.getItem("telephone").elements.data("aside")
							},
							"4": function() {},
							"5": function() {
				...
				validateFail: function(K) {
					var L, J, I;
					if (K == "noMobile") {
						L = E.validator.getItem("telephone")
					} else {
						if ("noImgCode") {
		...
		H = new GCH.TabChanger({
			container: "#tabs",
			tabDataName: "data-tab",
			onTabChange: function(K, M) {
				if (M.lastTab) {
					M.lastTab.removeClass("cur");
				}
				if (M.currentTab) {
					M.currentTab.addClass("cur");
				}
				...
				$("#telephone").attr("placeholder", M.tabName === "email" ? "请输入邮箱" : "请输入手机号");
				$(".G_input_error").removeClass("G_input_error");
...
GCH.oAuthLogin = (function() {
	...
	function J(L, N, M) {
		return "http://www." + L + "/UserIntergration/renren/?source=" + N + "&type=" + M
	}
	function B(N, M, L) {
		return "http://www.gouchehui.com.cn/UserIntergration/Renren/?key=" + N + "&type=" + L + "&returnurl=" + (typeof(M) === "function" ? M() : M)
	}
	function E() {
